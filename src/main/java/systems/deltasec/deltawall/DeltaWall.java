package systems.deltasec.deltawall;

import systems.deltasec.deltawall.threads.IpHandler;
...
public class DeltaWall {
    ...
    public static String PREFIX(){
        return "("+sdf.format(new Date())+")[DeltaWall-"+version+"] ";
    }
    ...
    public static void main(String[] args) {
        ...
        if(args.length != 6){
            System.out.println("Warning: DeltaWall has only been tested on Debian-Based Distributions.");
            System.out.println();
            System.out.println(DeltaWall.PREFIX() + "Correct Args: 'java -jar DeltaWall.jar [NET_INTERFACE] [MAX_THREADS] [MITIGATION_LENGTH_SECONDS] [PPS_TRIGGER] [MBPS_TRIGGER] [MAX_PACKETS_PER_IP]'");
        ...
        while(true){
            try {
                ...
                if (pps >= MitigationOptions.ppsCap && mbps >= MitigationOptions.mbpsCap) {
                    ...
                    System.out.println(DeltaWall.PREFIX() + "{ATTACK_ID="+mitigatedAttacks+"} (D)DOS Attack Detected: ["+mbps+"MBps] ["+pps+"pps]");

                    final long mitigationEnd = System.currentTimeMillis() + (MitigationOptions.mitigationLengthInSeconds * 1000);

                    System.out.println(DeltaWall.PREFIX() + "{ATTACK_ID="+mitigatedAttacks+"} (D)DOS Mitigation Started For "+MitigationOptions.mitigationLengthInSeconds+"s!");
                    ...
                    System.out.println(DeltaWall.PREFIX() + "{ATTACK_ID="+mitigatedAttacks+"} (D)DOS Mitigation Ended: [totalDropped="+Mitigation.droppedHosts.size()+"]");
